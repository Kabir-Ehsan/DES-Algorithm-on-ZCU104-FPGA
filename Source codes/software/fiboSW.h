 ...
 */
typedef ubyte subkey_tt[17][6]; /* 17 sets of 48 bits */


static void getSubKeys(const key_tt key, subkey_tt ks) {
 ...
 */
static void processMessage(const ubyte *message, subkey_tt ks, ubyte *ep) {
 ...
 */
String encrypt(const key_tt key, const ubyte *message, int len) {
    String result = {0,0};
    subkey_tt ks;
    ubyte padByte;
    int i;
...
/*
String decrypt(const key_tt key, const ubyte *message, int len) {
    String result = { 0, 0 };
    subkey_tt ks;
    int i, j;
    ubyte padByte;
 ...
 */
void driver(const key_tt key, const ubyte *message, int len) {
    String encoded;
...
int fiboSW() {
    const key_tt keys[] = {{0x13, 0x34, 0x57, 0x79, 0x9B, 0xBC, 0xDF, 0xF1}};//,
        //{0x0E, 0x32, 0x92, 0x32, 0xEA, 0x6D, 0x0D, 0x73},
        //{0x0E, 0x32, 0x92, 0x32, 0xEA, 0x6D, 0x0D, 0x73}
    //};


    const ubyte message0[] = {0x49, 0x6e, 0x20, 0x6f, 0x6c, 0x64, 0x65, 0x6e};
    const ubyte message1[] = {0x20, 0x74, 0x69, 0x6d, 0x65, 0x73, 0x20, 0x77};
    const ubyte message2[] = {0x68, 0x65, 0x6e, 0x20, 0x77, 0x69, 0x73, 0x68};
    const ubyte message3[] = {0x69, 0x6e, 0x67, 0x20, 0x73, 0x74, 0x69, 0x6c};
    const ubyte message4[] = {0x6c, 0x20, 0x64, 0x69, 0x64, 0x20, 0x73, 0x6f};
    const ubyte message5[] = {0x6d, 0x65, 0x20, 0x67, 0x6f, 0x6f, 0x64, 0x20};
    const ubyte message6[] = {0x74, 0x68, 0x65, 0x72, 0x65, 0x20, 0x6c, 0x69};

    const ubyte message296[] = {0x74, 0x68, 0x20, 0x6a, 0x6f, 0x79, 0x20, 0x77};
    const ubyte message297[] = {0x68, 0x65, 0x6e, 0x20, 0x73, 0x68, 0x65, 0x20};
    const ubyte message298[] = {0x73, 0x61, 0x77, 0x20, 0x68, 0x65, 0x72, 0x20};
    const ubyte message299[] = {0x62, 0x65, 0x61, 0x75, 0x74, 0x69, 0x66, 0x75};


    const ubyte message1012[] = {0x74, 0x20, 0x73, 0x74, 0x72, 0x61, 0x69, 0x67};
    const ubyte message1013[] = {0x68, 0x74, 0x20, 0x74, 0x6f, 0x20, 0x74, 0x68};
    const ubyte message1014[] = {0x65, 0x20, 0x63, 0x68, 0x75, 0x72, 0x63, 0x68};

    int len;

    len = sizeof(message0) / sizeof(ubyte);
    driver(keys[0], message0, len);

    len = sizeof(message1) / sizeof(ubyte);
    driver(keys[0], message1, len);

    len = sizeof(message2) / sizeof(ubyte);
    driver(keys[0], message2, len);

    len = sizeof(message3) / sizeof(ubyte);
    driver(keys[0], message3, len);

    len = sizeof(message4) / sizeof(ubyte);
    driver(keys[0], message4, len);

    len = sizeof(message5) / sizeof(ubyte);
    driver(keys[0], message5, len);

    len = sizeof(message6) / sizeof(ubyte);
    driver(keys[0], message6, len);

    len = sizeof(message296) / sizeof(ubyte);
    driver(keys[0], message296, len);

    len = sizeof(message297) / sizeof(ubyte);
    driver(keys[0], message297, len);

    len = sizeof(message298) / sizeof(ubyte);
    driver(keys[0], message298, len);

    len = sizeof(message299) / sizeof(ubyte);
    driver(keys[0], message299, len);

    len = sizeof(message1012) / sizeof(ubyte);
    driver(keys[0], message1012, len);

    len = sizeof(message1013) / sizeof(ubyte);
    driver(keys[0], message1013, len);

    len = sizeof(message1014) / sizeof(ubyte);
    driver(keys[0], message1014, len);
